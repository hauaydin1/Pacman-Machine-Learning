...
class Graph(object):
    ...
    def add(self, node):
        ...
        inputs = []
        self.nodes.append(node) # add this node in the Graph object

        for parent in parents:
            inputs.append(self.get_output(parent))
    
        node.output = node.forward(inputs) 
        node.gradient = np.zeros_like(node.output) # initialize an all-zero gradient accumulator
    ...
    def backprop(self):
        ...
        for node in self.nodes[::-1]: #for even nodes
            nodeIn = self.get_inputs(node) # node's backward method 
            outGrad = node.gradient
            retOut = node.backward(nodeIn, outGrad)

            for parent in range(0, len(node.get_parents())): # update the gradient accumulators for parents
                node.get_parents()[parent].gradient += retOut[parent]
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        input0 = inputs[0].T # transpose  
        input1 = inputs[1].T 
        a = np.dot(gradient, input1) # get dot product from transpose of matrices and gradient
        b = np.dot(input0, gradient)

        # return array list of length-2
        return [a, b]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        aMatrix = 0.5 * np.power(inputs[0] - inputs[1], 2) # 0.5 * (a[i,j] - b[i,j])^2 
        return np.mean(aMatrix) 

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        gradients1 = gradient * (inputs[0] - inputs[1]) # gradient * a[i,j] - b[i,j]
        gradients2 = gradient * (inputs[1] - inputs[0]) 

        shape = inputs[1].size # b has the same shape as a
        average1 = gradients1 / shape
        average2 = gradients2 / shape
